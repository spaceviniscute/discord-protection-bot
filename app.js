...
client.awaitReply = async (msg, question, limit = 60000) => {
  ...
  await msg.channel.send({ embeds: [question] });
  try {
...
client.on("guildMemberAdd", async (member) => {
  ...
  if (settings.type === "password") {
    ...
    if (response.includes(settings.password)) {
      ...
      member.roles.add(settings.role).catch((e) => console.log("Can't add role. Add the bot role higher"));
    } else {
      msg.reply(`Wrong password. Type \`${settings.prefix}password\` for another try.`);
    }
  } else if (settings.type === "captcha") {
    member.send(
      `This server is protected by captcha. Type \`${settings.prefix}verify\` in <#${
        member.guild.channels.cache.find((c) => c.name === settings.verificationChannel).id
      }>`
    );
  }
});
...
client.on("messageCreate", async (message) => {
  ...
  if (message.guild) {
    const verChan = message.guild.channels.cache.find((c) => c.name === settings.verificationChannel);
    if (verChan) {
  ...
  if (message.content.startsWith(`${settings.prefix}help`)) {
    message.delete().catch(() => console.log("Cannot delete message"));
    const embed = new MessageEmbed().setColor("1ABC9C").setTitle("Available Commands")
  ...
  if (message.content.startsWith(`${settings.prefix}verify`)) {
    ...
    const attachment = new MessageAttachment(await client.img(captcha.data).then((r) => r), "captcha.png");
    ...
    const m1 = await message.author.send("Generating captcha...").catch(async () => {
      ...
      return delmsg.delete().catch(() => console.log("Cannot delete message"));
    });
    m1.delete().catch(() => console.log("Cannot delete message"));
    ...
    if (response.includes(captcha.text)) {
      ...
      const role = await message.guild.roles.cache.find((r) => r.name.toLowerCase() === settings.role.toLowerCase());
      if (role) message.member.roles.add(role.id).catch((e) => console.log("Can't add role. Add the bot role higher"));
    } else {
      const logs = message.guild.channels.cache.find((c) => c.name === settings.logsChannel);
      if (logs) logs.send(`Failed captcha attempt by ${message.author.tag}`);
      return msg.reply(
        `Wrong captcha! Try again by typing \`${settings.prefix}verify\` again in <#${
          message.guild.channels.cache.find((c) => c.name === settings.verificationChannel).id
        }>`
      );
    }
  }
  if (message.content.startsWith(`${settings.prefix}password`)) {
    ...
    if (response.includes(settings.password)) {
      msg.reply("Successfully authenticated... Granting access...");
      const role = await message.guild.roles.cache.find((r) => r.name.toLowerCase() === settings.role);
      if (role) message.member.roles.add(role.id).catch((e) => console.log("Can't add role. Add the bot role higher"));
    } else {
      const logs = message.guild.channels.cache.find((c) => c.name === settings.logsChannel);
  ...
  if (message.content.startsWith(`${settings.prefix}config`)) {
    if (!message.member.permissions.has("ADMINISTRATOR"))
      return message.channel.send("Sorry, but you must have the `ADMINISTRATOR` permission to use this command.");
    if (message.channel.type === "dm") return message.channel.send("This command does not work via DM.");
    if (!args.length) {
      const embed = new MessageEmbed()
        .setColor("1ABC9C")
    ...
    if (key.toLowerCase() === "type" && !types.includes(args[1]))
      return message.reply("Protection type must be either `password` or `captcha`");
    if (key.toLowerCase() === "autoban" && !autoban.includes(args[1]))
      return message.reply("Autoban must be either `true` for enabled or `false` for disabled");
    if (
    ...
    if (
      ...
      !message.guild.channels.cache.find((r) => r.name.toLowerCase() === args[1].toLowerCase())
    )
  ...
  if (message.content.startsWith(`${settings.prefix}setup`)) {
    ...
    if (!re1)
      return message.channel.send("Setup waiting period expired. Type the setup command to go through it again!");
    const types = ["password", "captcha"];
    ...
    if (!role) {
      ...
      client.updateGuildSetting(message.guild, "role", re4);
      message.reply(`Verified role successfully set to \`${re4}\``);
      if (role.position > message.guild.me.roles.highest.position)
        message.channel.send("Move my role higher up in the role hierarchy or I won't be able to give out this role!");
    }
    ...
    if (res.includes(re5.toLowerCase())) {
      if (re5.toLowerCase() === "yes") {
        ...
        message.guild.channels.cache.forEach(async (channel, id) => {
          if (channel.name !== settings["verificationChannel"])
